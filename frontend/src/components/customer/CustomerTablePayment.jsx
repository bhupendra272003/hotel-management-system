...
export default function CustomerTablePayment() {
  ...
  const processPayment = async () => {
    ...
    setPaying(true);
    try {
      const response = await axios.post(`http://localhost:5000/api/billing/pay-table/${selectedBooking._id}`, {
  ...
  const styles = {
    container: {
      maxWidth: "700px", margin: "40px auto", padding: "35px",
      background: "linear-gradient(135deg, #ffffff 0%, #f8f9fa 100%)",
      borderRadius: "24px", boxShadow: "0 20px 40px rgba(0,0,0,0.1)",
      fontFamily: "'Inter', 'Segoe UI', sans-serif"
    },
    title: { textAlign: "center", color: "#1a1a2e", marginBottom: "10px", fontSize: "28px", fontWeight: "700" },
    subtitle: { textAlign: "center", color: "#666", marginBottom: "30px", fontSize: "14px" },
    input: { width: "100%", padding: "14px 16px", borderRadius: "12px", border: "2px solid #e8e8e8", fontSize: "15px" },
    ...
    modalContent: { background: "white", padding: "30px", borderRadius: "20px", maxWidth: "450px", width: "90%", boxShadow: "0 25px 50px rgba(0,0,0,0.3)", animation: "slideIn 0.3s ease" },
    select: { width: "100%", padding: "12px 16px", borderRadius: "10px", border: "2px solid #e0e0e0", fontSize: "15px", marginBottom: "15px", background: "white", cursor: "pointer" },
    bookingCard: { border: "1px solid #e0e0e0", borderRadius: "16px", padding: "20px", marginBottom: "15px", background: "white", boxShadow: "0 4px 12px rgba(0,0,0,0.05)" },
    ...
    successContainer: { textAlign: "center", padding: "30px", background: "linear-gradient(135deg, #d4edda 0%, #c3e6cb 100%)", borderRadius: "16px", marginBottom: "20px", animation: "fadeIn 0.5s ease" },
  ...
  return (
    <div style={styles.container}>
      ...
      <h2 style={styles.title}>🍽️ Complete Table Payment</h2>
      <p style={styles.subtitle}>Secure payment for your table booking</p>
            ...
            <div style={styles.bookingCard}>
              ...
              {orders.length > 0 && (
                <>
                  ...
                  {orders.map((order, idx) => (<div key={idx} style={{ marginLeft: "15px", fontSize: "14px", color: "#555" }}>• {order.name} x {order.quantity} = ₹{order.quantity * order.price}</div>))}
              ...
              {bookings.map((b) => {
                ...
                return (<div key={b._id} style={styles.bookingCard}>
                  ...
                  {b.paymentStatus !== "paid" && (<>
                    <select value={paymentMethod} onChange={(e) => setPaymentMethod(e.target.value)} style={{ ...styles.select, marginTop: "10px" }}>
