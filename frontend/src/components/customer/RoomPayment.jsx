...
export default function RoomPayment() {
  ...
  const processPayment = async () => {
    ...
    try {
      const response = await axios.post(`http://localhost:5000/api/billing/pay-room/${roomNo}`, {
        paymentMethod,
        transactionId,
      ...
      if (response.data.success) {
        setPaymentCompleted(true);
        setBill({ ...bill, paymentStatus: "paid", remainingAmount: 0 });
  ...
  const styles = {
    container: { maxWidth: "600px", margin: "40px auto", padding: "30px", background: "#ffffff", borderRadius: "20px", boxShadow: "0 10px 30px rgba(0,0,0,0.1)", fontFamily: "Arial, sans-serif" },
    title: { textAlign: "center", color: "#dc3c3c", marginBottom: "20px", fontSize: "28px" },
    inputGroup: { display: "flex", gap: "10px", marginBottom: "20px" },
    input: { flex: 1, padding: "12px", borderRadius: "8px", border: "2px solid #ddd", fontSize: "16px" },
    button: { padding: "12px 24px", background: "#2196f3", color: "white", border: "none", borderRadius: "8px", cursor: "pointer", fontSize: "16px", fontWeight: "bold" },
    payButton: { width: "100%", padding: "14px", background: "linear-gradient(135deg, #4caf50, #388e3c)", color: "white", border: "none", borderRadius: "8px", cursor: "pointer", fontSize: "16px", fontWeight: "bold", marginTop: "15px" },
    billCard: { border: "2px solid #eee", borderRadius: "12px", padding: "20px", marginBottom: "20px", background: "#f9f9f9" },
    billRow: { display: "flex", justifyContent: "space-between", padding: "8px 0", borderBottom: "1px solid #eee" },
    ...
    message: { padding: "12px", borderRadius: "8px", marginBottom: "20px", textAlign: "center" },
    ...
    select: { width: "100%", padding: "12px", borderRadius: "8px", border: "2px solid #ddd", fontSize: "16px", marginBottom: "10px", background: "white" },
    backButton: { width: "100%", padding: "12px", background: "#6c757d", color: "white", border: "none", borderRadius: "8px", cursor: "pointer", fontSize: "14px", marginTop: "10px" },
    successContainer: { textAlign: "center", padding: "20px", background: "#d4edda", borderRadius: "12px", marginBottom: "20px" }
  };

  return (
    <div style={styles.container}>
      <h2 style={styles.title}>🏠 Pay Room Bill</h2>
        ...
        <>
          ...
          {bill && bill.paymentStatus !== "paid" && (
            <div style={styles.billCard}>
              <h3 style={{ marginBottom: "15px", color: "#dc3c3c" }}>Bill Details</h3>
              ...
              <div style={{ marginTop: "20px" }}>
                ...
                <button onClick={processPayment} disabled={paying} style={{ ...styles.payButton, opacity: paying ? 0.7 : 1 }}>{paying ? "Processing..." : `Pay ₹${bill.remainingAmount?.toLocaleString()}`}</button>
              </div>
            </div>
          )}
        </>
