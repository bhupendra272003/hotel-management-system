...
export default function WaiterDashboard({ user }) {
  ...
  const fetchStats = async () => {
    try {
      ...
      setStats({
        ...
        completedToday: tasksData.filter(t => t.status === "completed" && 
  ...
  const addTaskNote = async () => {
    ...
    try {
      await axios.put(`http://localhost:5000/api/tasks/${selectedTask._id}`, {
        notes: taskNotes,
        status: "in-progress"
      });
  ...
  return (
    <div style={{ maxWidth: '1400px', margin: '0 auto', padding: '20px' }}>
      ...
      <div style={{
        ...
        <div>
          <h1 style={{ margin: 0, fontSize: '1.8rem' }}>🍽️ Waiter Dashboard</h1>
          <p style={{ margin: '5px 0 0', color: 'var(--text-secondary)' }}>
            Welcome, {user?.name || 'Waiter'}! Here are your tasks for today
          </p>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: '15px' }}>
          <Link to="/waiter/profile" style={{
            display: 'flex',
            alignItems: 'center',
            gap: '10px',
            padding: '10px 20px',
      ...
      <div style={{
        ...
        <div style={{
          ...
          <h3>Pending Tasks</h3>
          <p style={{ fontSize: '2rem', fontWeight: 'bold', margin: '10px 0' }}>{stats.pendingTasks}</p>
        </div>
        <div style={{
          ...
          <div style={{ fontSize: '2rem' }}>🔄</div>
          <h3>In Progress</h3>
          <p style={{ fontSize: '2rem', fontWeight: 'bold', margin: '10px 0' }}>{stats.inProgressTasks}</p>
        </div>
        <div style={{
          ...
          <p style={{ fontSize: '2rem', fontWeight: 'bold', margin: '10px 0' }}>{stats.completedToday}</p>
        </div>
        <div style={{
          ...
          <h3>Urgent Tasks</h3>
          <p style={{ fontSize: '2rem', fontWeight: 'bold', margin: '10px 0' }}>{stats.urgentTasks}</p>
        </div>
      </div>
                ...
                <div key={task._id} style={{
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'start', flexWrap: 'wrap', gap: '10px' }}>
                    <div>
                      ...
                      {task.roomNo && (
                        <p><strong>📍 Room:</strong> {task.roomNo}</p>
                      )}
                      {task.tableId && (
                        <p><strong>🍽️ Table:</strong> {task.tableId}</p>
                      )}
                      {task.notes && (
                    ...
                    <div style={{ textAlign: 'right' }}>
                      ...
                      <div style={{ display: 'flex', gap: '8px', marginTop: '10px' }}>
                        {task.status === "pending" && (
                          <>
                            <button onClick={() => updateTaskStatus(task._id, "in-progress")} style={{
                              padding: '6px 12px',
                              background: '#17a2b8',
                              color: 'white',
                              border: 'none',
                        ...
                        {task.status === "in-progress" && (
                          <>
                            <button onClick={() => updateTaskStatus(task._id, "completed")} style={{
                              padding: '6px 12px',
                              background: '#28a745',
                              color: 'white',
                              border: 'none',
      ...
      {showTaskModal && (
        <div style={{
          ...
          <div style={{
            ...
            <h3 style={{ color: '#dc3c3c', marginBottom: '20px' }}>
              {selectedTask?.notes ? "Update Task Note" : "Add Task Note"}
            </h3>
            <textarea
              value={taskNotes}
