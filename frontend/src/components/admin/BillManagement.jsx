...
export default function BillManagement() {
  ...
  return (
    <div style={{ maxWidth: "1400px", margin: "40px auto", padding: "20px" }}>
      ...
      {message && (
        <div style={{ padding: "12px 20px", borderRadius: "10px", marginBottom: "20px", textAlign: "center", backgroundColor: message.type === "success" ? "#d4edda" : "#f8d7da", color: message.type === "success" ? "#155724" : "#721c24", border: `1px solid ${message.type === "success" ? "#c3e6cb" : "#f5c6cb"}` }}>
          {message.text}
        </div>
      )}
      
      <div style={{ display: "flex", justifyContent: "flex-end", marginBottom: "20px" }}>
        <button onClick={fetchBills} style={{ padding: "10px 20px", background: "linear-gradient(135deg, #28a745, #20c997)", color: "white", border: "none", borderRadius: "10px", cursor: "pointer", display: "flex", alignItems: "center", gap: "8px" }}>
          <span>🔄</span> Refresh
        </button>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(280px, 1fr))", gap: "25px", marginBottom: "40px" }}>
        ...
        <div style={{ background: "linear-gradient(135deg, #28a745 0%, #20c997 100%)", padding: "25px", borderRadius: "20px", color: "white", boxShadow: "0 10px 25px rgba(0,0,0,0.1)", transition: "transform 0.3s ease" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <div>
              <div style={{ fontSize: "14px", opacity: "0.8", marginBottom: "5px" }}>Paid Amount</div>
              <div style={{ fontSize: "32px", fontWeight: "bold" }}>₹{stats.paid.toLocaleString()}</div>
            </div>
            <div style={{ fontSize: "40px" }}>✅</div>
          </div>
        </div>
        <div style={{ background: "linear-gradient(135deg, #dc3545 0%, #c82333 100%)", padding: "25px", borderRadius: "20px", color: "white", boxShadow: "0 10px 25px rgba(0,0,0,0.1)", transition: "transform 0.3s ease" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <div>
              <div style={{ fontSize: "14px", opacity: "0.8", marginBottom: "5px" }}>Unpaid Amount</div>
              <div style={{ fontSize: "32px", fontWeight: "bold" }}>₹{stats.unpaid.toLocaleString()}</div>
            </div>
            <div style={{ fontSize: "40px" }}>⏳</div>
          </div>
        </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: "20px", marginBottom: "30px" }}>
        <div onClick={() => setFilter("room")} style={{ background: "linear-gradient(135deg, #ffecd2 0%, #fcb69f 100%)", padding: "20px", borderRadius: "15px", cursor: "pointer", transition: "transform 0.3s ease", textAlign: "center", boxShadow: "0 5px 15px rgba(0,0,0,0.08)" }}>
          <div style={{ fontSize: "36px" }}>🏠</div>
          <div style={{ fontSize: "18px", fontWeight: "bold", color: "#333", marginTop: "10px" }}>Room Revenue</div>
          <div style={{ fontSize: "20px", fontWeight: "bold", color: "#dc3545", marginTop: "5px" }}>₹{stats.roomTotal.toLocaleString()}</div>
        </div>
        <div onClick={() => setFilter("food")} style={{ background: "linear-gradient(135deg, #d4edda 0%, #a8e6cf 100%)", padding: "20px", borderRadius: "15px", cursor: "pointer", transition: "transform 0.3s ease", textAlign: "center", boxShadow: "0 5px 15px rgba(0,0,0,0.08)" }}>
          <div style={{ fontSize: "36px" }}>🍕</div>
          <div style={{ fontSize: "18px", fontWeight: "bold", color: "#333", marginTop: "10px" }}>Food Revenue</div>
          <div style={{ fontSize: "20px", fontWeight: "bold", color: "#28a745", marginTop: "5px" }}>₹{stats.foodTotal.toLocaleString()}</div>
        </div>
        <div onClick={() => setFilter("table")} style={{ background: "linear-gradient(135deg, #d1ecf1 0%, #bee5eb 100%)", padding: "20px", borderRadius: "15px", cursor: "pointer", transition: "transform 0.3s ease", textAlign: "center", boxShadow: "0 5px 15px rgba(0,0,0,0.08)" }}>
          <div style={{ fontSize: "36px" }}>🍽️</div>
          <div style={{ fontSize: "18px", fontWeight: "bold", color: "#333", marginTop: "10px" }}>Table Revenue</div>
          <div style={{ fontSize: "20px", fontWeight: "bold", color: "#17a2b8", marginTop: "5px" }}>₹{stats.tableTotal.toLocaleString()}</div>
        </div>
      </div>
      ...
      <div style={{ display: "flex", gap: "12px", flexWrap: "wrap", marginBottom: "25px", justifyContent: "center" }}>
        {["all", "room", "food", "table", "paid", "unpaid"].map((f) => (
          <button key={f} onClick={() => setFilter(f)} style={{ padding: "8px 20px", background: filter === f ? "#dc3c3c" : "#f0f0f0", color: filter === f ? "white" : "#333", border: "none", borderRadius: "25px", cursor: "pointer", fontSize: "14px", fontWeight: "500", transition: "all 0.3s ease" }}>
      ...
      {filteredBills.length === 0 ? (
        <div style={{ textAlign: "center", padding: "60px", background: "#f8f9fa", borderRadius: "15px" }}>
          ...
          <p style={{ color: "#666", fontSize: "16px" }}>No bills found</p>
        </div>
      ) : (
          ...
          <table style={{ width: "100%", borderCollapse: "collapse", background: "white", fontSize: "14px" }}>
            <thead>
              <tr style={{ background: "linear-gradient(135deg, #dc3c3c, #b83232)", color: "white" }}>
                ...
                <th style={{ padding: "15px", textAlign: "left" }}>Room/Table</th>
              ...
              {filteredBills.map((bill, idx) => (
                <tr key={bill._id} style={{ borderBottom: "1px solid #e0e0e0", background: idx % 2 === 0 ? "#fafafa" : "white" }}>
                  ...
                  <td style={{ padding: "12px 15px" }}>
                    <span style={{ padding: "5px 12px", borderRadius: "20px", fontSize: "12px", fontWeight: "600", background: bill.paymentStatus === "paid" ? "#d4edda" : "#f8d7da", color: bill.paymentStatus === "paid" ? "#155724" : "#721c24" }}>
      ...
      {selectedBill && (
        <div style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, background: "rgba(0,0,0,0.7)", display: "flex", justifyContent: "center", alignItems: "center", zIndex: 1000, backdropFilter: "blur(5px)" }} onClick={() => setSelectedBill(null)}>
          <div style={{ background: "white", padding: "35px", borderRadius: "20px", maxWidth: "500px", width: "90%", boxShadow: "0 25px 50px rgba(0,0,0,0.3)" }} onClick={(e) => e.stopPropagation()}>
